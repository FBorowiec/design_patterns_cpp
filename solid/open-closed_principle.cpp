  ...
  std::vector<Product*>BySizeAndColor(std::vector<Product*> products, Size size, Color color)
  {
    ...
    for (auto& p : products)
      if (p->size == size && p->color == color)
        products_by_size_and_color.push_back(p);
...
template <typename T> struct AndSpecification : Specification<T>
{
  ...
  {
    return first_spec.IsSatisfied(item) && second_spec.IsSatisfied(item);
  }
...
TEST(OpenClosedPrincipleTest, ExampleOfClassFollowingTheOCPrinciple)
{
  ...
  for (auto& item : bf.FilterWithSpec(items, green))
    std::cout<< item->name << " is green\n";

  good::SizeSpecification large(Size::large);
  good::AndSpecification<Product> green_and_large(green, large);

  for (auto& item : bf.FilterWithSpec(items, green_and_large))
    std::cout<< item->name << " is green and large\n";
}
